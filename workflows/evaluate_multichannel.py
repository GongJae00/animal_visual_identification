...
def cmd_protected(args: argparse.Namespace) -> None:
    provenance = build_offline_tool_provenance(
        Path(__file__),
        additional_paths=(
            Path(__file__).resolve().parents[1] / "evaluation" / "protected_evaluation.py",
            Path(__file__).resolve().parents[1] / "foundation" / "protected_io.py",
            Path(__file__).resolve().parents[1] / "evaluation" / "retrieval.py",
        ),
    )
