...
class BuilderSourceProvenanceRow:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "BuilderSourceProvenanceRow":
        if set(payload) not in (
            {"relative_path", "content_sha256", "byte_size"},
            {"logical_name", "relative_path", "content_sha256", "byte_size"},
        ):
...
class BuilderToolProvenance:
    ...
    def __post_init__(self) -> None:
        ...
        if self.schema_version == "cvi.offline_tool_provenance.v1":
            ...
            required_families = (
                ...
                {
                    "embedding/methods/classical/pdq/contracts.py",
                    "embedding/methods/classical/pdq/native.py",
                    "embedding/methods/classical/pdq/source_intake.py",
                    "workflows/build_native_pdq_worker.py",
                },
            )
            if (
                ...
                or "workflows.build_native_pdq_worker" not in self.entrypoints
    ...
    def from_dict(cls, payload: dict[str, Any]) -> "BuilderToolProvenance":
        ...
        if set(payload) not in (expected, v2_expected):
        ...
        if not isinstance(entrypoints, list):
            raise TypeError("builder provenance entrypoints differ")
        return cls(
            ...
            logical_component=payload.get("logical_component"),
...
def _current_builder_tool_provenance() -> dict[str, Any]:
    ...
    return build_offline_tool_provenance(
        repository_root / "workflows" / "build_native_pdq_worker.py",
        logical_component="embedding.methods.classical.pdq.native",
    )
