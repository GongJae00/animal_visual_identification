"""Evaluate bounded robust aggregation candidates on fixed-panel N3 embeddings."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--panel-sha256",
        required=True,
        help="external canonical JSON SHA-256 of the fixed panel bundle",
    )
    parser.add_argument("--topology-manifest", type=Path, required=True)
    parser.add_argument(
        "--topology-sha256",
        required=True,
        help="external canonical JSON SHA-256 of the exact topology manifest",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    from archive.nose_metric.experiments.n4_robust_nose import evaluate_n4_robust_nose
        ...
        json.dumps(
            {
                ...
                "selected_candidate": report["development"]["selection"][
                    "selected_candidate"
                ],
                "evaluation_metrics": report["evaluation"][
