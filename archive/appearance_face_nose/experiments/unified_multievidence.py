...
_CODE_PATHS = (
    "archive/appearance_face_nose/experiments/unified_multievidence.py",
    ...
    "archive/appearance_face_nose/commands/evaluate_yt_unified_multievidence.py",
)
_PRE_PARSING_CODE_PATHS = (
    "archive/appearance_face_nose/experiments/unified_multievidence.py",
    "evaluation/search_metrics/metrics.py",
    "identification/export/appearance/evidencer.py",
    "shared/contracts/artifact_manifest.py",
...
_PRE_TRAINING_OWNERSHIP_CODE_PATHS = tuple(
    ...
    if path == "identification/training/nose/embedding_consistency_training.py"
    else path
    for path in _PRE_SHARED_CODE_PATHS
)
_PRE_NESTED_EMBEDDING_CODE_PATHS = tuple(
    path.replace("identification/export/nose/signal/", "embedding/methods/nose/", 1)
    if path.startswith("identification/export/nose/signal/")
    else path
...
def validate_report_bundle(bundle: object) -> dict[str, Any]:
    ...
    if code_paths not in {
        ...
        frozenset(_PRE_PARSING_CODE_PATHS),
        frozenset(_PRE_SHARED_CODE_PATHS),
...
def evaluate_unified_multievidence(
    *,
    ...
    repository = find_repo_root(__file__) if repository_root is None else repository_root.resolve(strict=True)
