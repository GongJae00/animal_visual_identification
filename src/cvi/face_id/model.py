...
class FaceRegionalEncoder(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.projection = nn.Sequential(
            nn.Linear(384 * 10, 512), nn.GELU(), nn.LayerNorm(512),
            nn.Linear(512, 256),
        )

    def _region_masks(self, h: int, w: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        y, x = torch.meshgrid(
    ...
    def forward(self, hidden_states: Sequence[torch.Tensor]) -> torch.Tensor:
        ...
        regions = self._region_masks(16, 16, device=features.device, dtype=features.dtype)
        power = torch.tensor(2.0, device=features.device)
        ...
        for region_idx in range(5):
            ...
            channel_attn = torch.softmax(
                features.mean(dim=1, keepdim=True).flatten(2), dim=-1
            ).view(b, 1, 16, 16)
...
class FaceIDModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.quality_head = nn.Sequential(nn.Linear(256, 64), nn.GELU(), nn.Linear(64, 1))
    ...
    def forward(self, rgb: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        embedding = self.encoder(hidden)
        quality = torch.sigmoid(self.quality_head(embedding.detach()))
