...
class SubCenterArcFace(nn.Module):
    ...
    def forward(
        ...
        phi = target * math.cos(margin) - torch.sqrt(1.0 - target.square()) * math.sin(margin)
        logits = cosine.clone()
...
class NoseIDObjective(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        consistency = embedding.sum() * 0.0 if second_view_embedding is None else view_consistency_loss(embedding, second_view_embedding)
        ...
        quality_aux = embedding.sum() * 0.0 if degradation_target is None else F.smooth_l1_loss(output["degradation_predictions"], degradation_target)
        total = arc + 0.4 * supcon + 0.2 * triplet + 0.1 * consistency + 0.1 * branch_aux + 0.05 * quality_aux
        return {
            "total": total,
