...
class CVI:
    def __init__(self, config: dict[str, Any] | str | Path | None = None):
        if config is None:
            raise ValueError("an explicit CVI configuration is required")
        ...
        if not isinstance(config, dict):
            raise ValueError("CVI configuration must be a JSON object")
        ...
        if unknown_keys:
            raise ValueError(
                f"unknown CVI configuration keys: {sorted(unknown_keys)}"
            )
    ...
    def _build_evidence(self) -> dict[str, Any]:
        ...
        from cvi.evidence.nose_print import NosePrintExtractor, NoseRoiPolicy
...
def _read_strict_json_object(
    ...
    *,
    ...
    if no_follow is None:  # pragma: no cover - the supported release target is Linux
        raise RuntimeError("secure CVI configuration reads require O_NOFOLLOW support")
    try:
