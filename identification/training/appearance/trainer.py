...
from identification.training.appearance.config import TrainConfig
...
def _build_label_index(rows: tuple[Any, ...]) -> dict[str, int]:
...
def evaluate_pretrained_development(
    config: TrainConfig,
    crop_root: Path,
    admission_manifest: Any,
    crop_manifest: PublicCropManifest,
    admission_receipt: Any,
    *,
    exposure_ledger: Any,
    exposure_receipt: Any,
    verify_admission: Callable[..., Any],
...
def train_model(
    ...
    *backbone_factory* is a callable(embedding_dim, use_gradient_checkpointing) → nn.Module.
    ...
    verified_admission = verify_admission(
        admission_manifest,
        crop_manifest,
        admission_receipt,
        crop_root=crop_root,
