...
def train_and_evaluate_masked_afn(
    *,
    ...
    source_bundle_from_dict: Callable[..., Any] | None = None,
) -> dict[str, Any]:
    ...
    if source_bundle_path is not None and image_content_receipts_path is not None:
        if source_bundle_from_dict is None:
            raise ValueError("source_bundle_from_dict is required with a source bundle")
        source_resolver, source_binding = _load_source_token_resolver(
            source_bundle_path=source_bundle_path,
            image_content_receipts_path=image_content_receipts_path,
            source_bundle_from_dict=source_bundle_from_dict,
        )
    ...
    try:
        ...
        for fold_index in range(kfold.policy.fold_count):
            view = materialize_fold(kfold, fold_index)["view"]
            sample_roles = {
...
def _load_source_token_resolver(
    *,
    ...
    source_bundle_from_dict: Callable[..., Any],
) -> tuple[
    ...
    source_bundle = source_bundle_from_dict(source_document.payload)
...
def _train_region_adapter(
    ...
    *,
    ...
    from identification.training.appearance.trainer import ArcFaceHead
