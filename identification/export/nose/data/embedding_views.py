...
_SHA256_PATHS = (
    "identification/export/nose/data/embedding_views.py",
    "parsing/export/regions/native_yt.py",
    "parsing/training/regions/segmentation_training.py",
    "shared/contracts/artifact_manifest.py",
    "shared/foundation/protected_io.py",
    "shared/foundation/protected_publication.py",
    "shared/foundation/provenance.py",
    "archive/nose/commands/prepare_nose_embedding_views.py",
)
_PRE_PARSING_SHA256_PATHS = (
    "identification/export/nose/data/embedding_views.py",
    "parsing/nose_region/native_yt.py",
    "parsing/nose_region/segmentation_training.py",
    "shared/contracts/artifact_manifest.py",
    "shared/foundation/protected_io.py",
    "shared/foundation/protected_publication.py",
    "shared/foundation/provenance.py",
...
_PRE_EMBEDDING_SHA256_PATHS = tuple(
    ...
    if path == "identification/export/nose/data/embedding_views.py"
    else path
    for path in _PRE_SHARED_SHA256_PATHS
)
_LEGACY_SHA256_PATHS = tuple(
    path.replace("parsing/", "localization/", 1)
    if path.startswith("parsing/")
    else path
    for path in _PRE_EMBEDDING_SHA256_PATHS
)
_RECORD_FIELDS = {
    "sample_token",
    "registered_dog_id",
...
def prepare_embedding_views(
    *,
    ...
    from parsing.training.regions.segmentation_training import validate_lineage_manifest
...
def validate_embedding_views_manifest(
    ...
    *,
    ...
    if code_paths not in {
        frozenset(_SHA256_PATHS),
        frozenset(_PRE_PARSING_SHA256_PATHS),
        frozenset(_PRE_SHARED_SHA256_PATHS),
        frozenset(_PRE_EMBEDDING_SHA256_PATHS),
        frozenset(_LEGACY_SHA256_PATHS),
    }:
        raise ValueError("embedding view code hash set differs")
    for name, digest in code.items():
        _require_sha256(digest, f"code_sha256s.{name}")
    if verify_code_sources:
        repository = (
            Path(__file__).resolve().parents[4]
            if repository_root is None
            else repository_root.resolve(strict=True)
