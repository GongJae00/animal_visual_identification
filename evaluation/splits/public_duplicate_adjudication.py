...
from shared.contracts.geometric_evidence import (
    GeometricVerifierEvidenceContract,
)
from shared.contracts.pdq import PDQSearchPolicy, PDQSearchResult
...
_CANDIDATE_HASH_BLOCK_SIZE = 4_096
...
def build_adjudication_chunk(
    *,
    ...
    geometric_evidence: Sequence[GeometricVerifierEvidenceContract] = (),
    ...
    candidate_set_sha256 = _candidate_set_sha256(ordered_pairs, pair_channels)
    ...
    for pair in ordered_pairs[start_index:end_index]:
        ...
        if conservative:
            ...
            if exact is not None or "PDQ" in channels or (
                ...
                and geometry.decision.value == "GEOMETRIC_CONFIRMED"
            ):
                outcome = CandidateOutcome.CANDIDATE_COMPONENT_DEPENDENCY
                reason = (
            ...
            if geometry.decision.value == "GEOMETRIC_CONFIRMED":
                outcome = CandidateOutcome.GEOMETRIC_CONFIRMED
                reason = geometry.reason.value
            elif geometry.decision.value == "GEOMETRIC_REJECTED":
...
def assemble_frozen_evidence_graph(
    *, source: PublicSplitSourceBundle, ledger: AdjudicationLedger
    ...
    if ledger.mode is AdjudicationMode.STANDARD:
...
def _candidate_set_sha256(
    ...
    pair_channels: Mapping[tuple[str, str], Mapping[str, str]],
) -> str:
    digest = hashlib.sha256(b"[")
    for start in range(0, len(ordered_pairs), _CANDIDATE_HASH_BLOCK_SIZE):
        if start:
            digest.update(b",")
        rows = [
            {
            ...
            for pair in ordered_pairs[start:start + _CANDIDATE_HASH_BLOCK_SIZE]
        ]
...
def _geometry_results(
    evidence: Sequence[GeometricVerifierEvidenceContract],
    sample_by_opaque: Mapping[str, str],
