...
def summarize_duplicates(
    ...
    samples_by_path: dict[str, list[UnifiedCanidSample]] = defaultdict(list)
    for sample in samples:
        samples_by_path[sample.image_path].append(sample)
    representatives = tuple(rows[0] for rows in samples_by_path.values())
    duplicates = find_exact_duplicates(representatives, data_root)
    repeated_paths = [rows for rows in samples_by_path.values() if len(rows) > 1]
    return {
        "total_samples": len(samples),
        "unique_image_paths": len(samples_by_path),
        "repeated_source_path_groups": len(repeated_paths),
        "repeated_source_path_samples": sum(len(rows) for rows in repeated_paths),
