...
class DogDetector:
    ...
    def detect_dogs(self, image: Image.Image) -> list[Detection]:
        ...
        dogs = [d for d in all_dets if d.class_name == "dog"]
        return suppress_contained_duplicates(dogs, iou_threshold=self._cfg.iou_threshold)
    ...
    def compute_quality(image: Image.Image,
        ...
        quality_image = (
            image
            if det is None
            else image.crop((det.x1, det.y1, det.x2, det.y2))
        )
        arr = np.array(quality_image.convert("L"), dtype=np.float32)
        if arr.size == 0:
            raise ValueError("quality detection crop must be non-empty")
...
class FrameSelector:
    ...
    def select(self, frames: list[tuple[int, Image.Image, Detection]]
        ...
        for idx, img, det in frames:
            ...
            if q.sharpness < self._min_sharpness:
                continue
...
def suppress_contained_duplicates(
    ...
    """Remove lower-confidence proposals that almost entirely duplicate one dog."""
    ...
    for candidate in sorted(
        detections,
        key=lambda item: (-item.confidence, -item.area, item.x1, item.y1, item.x2, item.y2),
    ):
        duplicate = False
        for accepted in kept:
            intersection_width = max(
                0, min(candidate.x2, accepted.x2) - max(candidate.x1, accepted.x1)
            )
            intersection_height = max(
                0, min(candidate.y2, accepted.y2) - max(candidate.y1, accepted.y1)
            )
            intersection = intersection_width * intersection_height
            union = candidate.area + accepted.area - intersection
            iou = intersection / max(union, 1)
            containment = intersection / max(min(candidate.area, accepted.area), 1)
            if iou >= iou_threshold and containment >= 0.90:
                duplicate = True
                break
        if not duplicate:
