...
Usage:
    uv run python -m data.commands.download datasets
    uv run python -m data.commands.download datasets --dataset dogfacenet
    uv run python -m data.commands.download datasets --dataset yt-bb-dog
    uv run python -m data.commands.download datasets --list
    uv run python -m data.commands.download datasets create-manifest ...
    uv run python -m data.commands.download datasets check MANIFEST
    uv run python -m data.commands.download datasets camera-spec ...
...
Dataset files resolve from ``CANINE_IDENTITY_DATASETS_DIR`` when set,
otherwise ``<data-root>/datasets``.
"""
...
from shared.contracts.model_paths import DATA_DIR, DATASETS_DIR, SUPPORTED_DATASETS
...
def _run_download(argv: list[str]) -> None:
    ...
    parser.add_argument(
        "--data-root",
        default=None,
        help="Data root directory (default: CANINE_IDENTITY_DATA_DIR or ~/canine_identity_data). Dataset files use CANINE_IDENTITY_DATASETS_DIR when set.",
    )
    parser.add_argument(
        "--list",
        action="store_true",
