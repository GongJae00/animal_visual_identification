...
from data.public_sources.public_crop_manifest import (
    PublicCropArtifact,
    PublicCropManifest,
...
from evaluation.splits.training_admission import (
    TrainingAdmissionManifest,
    TrainingCropRow,
    admit_training,
    verify_training_admission_receipt,
)
from identification.training.appearance.augment import RandAugment
...
class AdmittedCropDatasetTests(unittest.TestCase):
    ...
    def test_sampler_labels_do_not_decode_uncached_images(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "identification.training.appearance.trainer.read_verified_crop_artifact",
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, "byte size mismatch"):
                train_model(
                    ...
                    exposure_ledger=exposure_ledger,
                    exposure_receipt=exposure_receipt,
                    verify_admission=verify_training_admission_receipt,
...
class AugmentationContractTests(unittest.TestCase):
    def test_each_randaugment_operation_is_clamped_to_image_range(self) -> None:
        ...
        with patch(
            "identification.training.appearance.augment.random.choice", side_effect=operations
        ), patch("identification.training.appearance.augment.random.uniform", return_value=0.5):
...
class MultiHeadTrainingContractTests(unittest.TestCase):
    def test_embedding_trainer_requires_immutable_admission_inputs(self) -> None:
        completed = subprocess.run(
            [
                sys.executable,
                "identification/commands/train.py",
                "--crop-root", "crops",
                "--output-dir", "output",
            ],
            cwd=REPO_ROOT,
    ...
    def test_embedding_trainer_refuses_existing_output_before_input_reads(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            output.mkdir()
            command = [sys.executable, "identification/commands/train.py"]
            for argument in (
                "admission-manifest",
                "admission-receipt",
                "crop-manifest",
...
class TrainingArtifactTests(unittest.TestCase):
    ...
    def test_checkpoint_reconstructors_preserve_backbone_error_precedence(self) -> None:
        from archive.shared_helpers.commands import evaluate_roi_reid
...
class ArcFaceModelTests(unittest.TestCase):
    ...
    def test_dinov2_compat_subclass(self) -> None:
        from identification.training.appearance.trainer import Dinov2ArcFaceModel
        cfg = TrainConfig(embedding_dim=64, num_classes=5)
...
class ConvNeXtOnnxExportTests(unittest.TestCase):
    def test_export_and_reload(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".onnx") as f:
            ...
            from identification.export.backbones.extractors import OnnxExtractor
