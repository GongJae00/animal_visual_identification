...
def _fixture(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict[str, object]:
    ...
    from parsing.training.regions import segmentation_training
...
def test_student_masked_rgb_matches_architecture_pixels(
    ...
    import archive.nose.experiments.nose_architecture as architecture
...
def test_prepare_cli_help() -> None:
    completed = subprocess.run(
        [sys.executable, "archive/nose/commands/prepare_nose_embedding_views.py", "--help"],
        check=True,
        capture_output=True,
        text=True,
    )
