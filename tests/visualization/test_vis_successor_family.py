...
def _patch_governance_validators(monkeypatch: pytest.MonkeyPatch) -> None:
    import archive.full128.evaluation.full128_successors as successors
    import evaluation.splits.face.face_gallery_query_panel as gallery_panel
    import evaluation.splits.face.face_identity_protocol_v2 as protocol_v2
    from archive.full128.methods import face_visible
...
def test_successor_family_workflow_help_exposes_named_artifacts() -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(root / "archive/shared_helpers/commands/render_research_visualizations.py"),
            "--help",
        ],
        check=True,
        capture_output=True,
