...
from data.audit.pdq.contracts import PDQ_D4_ORIENTATIONS, PDQFingerprint
...
WORKER_SOURCE = REPO_ROOT / "data/audit/pdq/native_worker/main.cpp"
...
def _builder_provenance() -> dict[str, object]:
    return build_offline_tool_provenance(
        REPO_ROOT / "archive/shared_helpers/commands/build_native_pdq_worker.py"
    )
...
class NativePdqIntegrationTests(unittest.TestCase):
    ...
    def test_build_receipt_rejects_self_consistent_provenance_tampering(self) -> None:
        ...
        provenance_mutations = {
            ...
            "missing CLI": lambda value: value.__setitem__(
                "code_source_files",
                [row for row in value["code_source_files"] if row["relative_path"] != "archive/shared_helpers/commands/build_native_pdq_worker.py"],
            ),
        }
    ...
    def test_build_rejects_mid_build_repository_provenance_change(self) -> None:
        ...
        with TemporaryDirectory() as temporary, mock.patch(
            "data.audit.pdq.native._current_builder_tool_provenance",
...
class NativePdqContractTests(unittest.TestCase):
    def test_builder_provenance_accepts_only_complete_path_generations(self) -> None:
        ...
        native = next(
            ...
            if row["relative_path"] == "data/audit/pdq/native.py"
        )
        native["relative_path"] = "embedding/methods/classical/pdq_native.py"
        native["logical_name"] = "data.audit.pdq_native"
