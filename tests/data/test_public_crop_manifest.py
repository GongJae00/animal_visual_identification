...
class PublicCropManifestTests(unittest.TestCase):
        ...
        with TemporaryDirectory() as temporary:
            ...
            for label, manifest, policy, message in cases:
                with self.subTest(cap=label):
                    with (
                        patch(
                            "data.public_sources.public_crop_manifest._read_exact_regular_file"
                        ) as read_file,
                        patch(
                            "data.public_sources.public_crop_manifest._verify_decoded_image"
                        ) as decode_image,
                    ):
    ...
    def test_encoded_stat_cap_and_decoded_header_cap_precede_expensive_work(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            with patch("data.public_sources.public_crop_manifest.os.read") as read_file:
