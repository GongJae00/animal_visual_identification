...
from operations.video.decode import (
    DecodeBackend,
    DecodeConfig,
    DecodeRun,
    benchmark_decode,
...
class DecodeTests(unittest.TestCase):
    ...
    def test_injected_gpu_monitor_preserves_summary(self) -> None:
        ...
        with (
            patch(
                "operations.video.decode.run_decode_once",
                return_value=(run, ("ffmpeg",)),
            ),
            patch(
                "operations.video.decode.probe_video_file",
                return_value=SimpleNamespace(average_fps=30.0),
            ),
            patch(
                "operations.video.decode.subprocess.run",
