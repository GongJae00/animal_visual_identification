...
def test_exclusive_ownership_assigns_contact_pixels_once() -> None:
    ...
    drafts = [
        _DraftInstance(first, first.source_box_xyxy, first.probability, first_score, first_mask, 1.0, False),
        _DraftInstance(second, second.source_box_xyxy, second.probability, second_score, second_mask, 1.0, False),
    ]
...
class _InstanceRuntime:
    ...
    def predict_all(self, image: Image.Image, **_: object) -> tuple[AnimalInstanceCandidate, ...]:
