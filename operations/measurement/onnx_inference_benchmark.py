...
from operations.measurement.telemetry import GpuTelemetrySummary, monitor_operation
...
_BENCHMARK_WORKER_MODULES = {
    "operations.onnx_inference_benchmark",
    "operations.measurement.onnx_inference_benchmark",
}
_CURRENT_BENCHMARK_WORKER_MODULE = "operations.measurement.onnx_inference_benchmark"
        ...
        def run_workers() -> tuple[
            ...
            for index in range(policy.fresh_processes):
                ...
                command = (
                    ...
                    "-c",
                    ISOLATED_WORKER_BOOTSTRAP,
                    _CURRENT_BENCHMARK_WORKER_MODULE,
                    str(request_path),
                    "--worker-request",
                    str(request_path),
                    "--worker-result",
...
def _validate_worker_result(
    ...
    *,
    ...
    from prototype.export.embedding_producer import EmbeddingBackendIdentity
...
def _validate_summary_aggregation(
    ...
    for item in summary.worker_results:
        ...
        if (
            ...
            or command[1:5] != ("-I", "-B", "-c", ISOLATED_WORKER_BOOTSTRAP)
            or command[5] not in _BENCHMARK_WORKER_MODULES
