"""Generate publication-oriented, privacy-safe CVI report visualizations."""
...
def _pipeline_boundary_scene() -> Scene:
    scene = Scene("FIG 03", "Runtime boundary, made explicit", "Research localization is upstream; the canonical public API is crop-level.", "Source: README + src/cvi/api.py + localization modules | Dashed branch is not connected product capability")
    ...
    scene.text(1120, 250, "CANONICAL CVI", 18, LIME, bold=True)
    _box(scene, 1140, 450, 560, 170, "Caller-provided crop", ("CVI starts here", "PIL.Image enrollment / search"), LIME)
...
def _retrieval_scene() -> Scene:
    scene = Scene("FIG 06", "Gallery retrieval contract", "Template scores become stable identity-level closed-set candidates.", "Source: src/cvi/evaluation/retrieval.py | Canonical aggregation=max; alternatives are research-only and not selected")
    boxes = (
...
def render_preprocessing(record: Mapping[str, Any], paths: Mapping[str, Path], path: Path) -> None:
    image, draw = _base_raster("FIG 02", "Experimental preprocessing trace", "One hash-verified AP-10K record: source -> dog -> face -> weak nose -> source-valid mask.", "Source: AP-10K + experimental ROI manifest | Localization is upstream research, not canonical CVI runtime")
    ...
    draw.text((780, 900), "All displayed crops are derivatives; no raw source file is copied into Visualization/.", font=_font(17), fill=MUTED)
    path.parent.mkdir(parents=True, exist_ok=True)
    image.save(path, format="PNG", compress_level=9, optimize=False)


def _index_text() -> str:
    return """# CVI Report Visualization Index
...
def _write_provenance(root: Path, selected: Sequence[SelectedImage], report_path: Path) -> None:
    ...
    payload = {
        ...
        "generator": {
            "source": "tools/generate_report_visualizations.py",
