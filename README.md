...
연구 코드는 `contracts/`, `data_pipeline/`, `identity_governance/`, `localization/`, `identity_methods/`, `representation_learning/`, `evidence_fusion/`, `identity_retrieval/`, `evaluation/`, `experiments/`로 분리되어 있습니다. 실행 정의는 `workflows/`, 주요 tracked config는 각 기능 package와 `experiments/configs/`에 있습니다. Raw data, checkpoint, gallery, cache, run result는 Git 밖에 둡니다.
