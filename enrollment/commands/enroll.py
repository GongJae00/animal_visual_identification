...
Commands: build (default), bind, check, augment-labels.
"""
...
def _run_augment_labels(argv: list[str]) -> None:
    parser = argparse.ArgumentParser(
        description=(
...
def main(argv: list[str] | None = None) -> None:
    ...
    command = "build"
    if argv and argv[0] in {"build", "bind", "check", "augment-labels"}:
        command = argv[0]
        argv = argv[1:]
    {
        "build": _run_build,
        "bind": _run_bind,
        "check": _run_check,
        "augment-labels": _run_augment_labels,
    }[command](argv)
