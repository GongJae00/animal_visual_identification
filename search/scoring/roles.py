...
class AvailableIntersectionScorer:
    """Exact weighted cosine over channels available in both query and gallery key."""
    ...
    def __init__(self, channels: Sequence[EvidenceChannelSpec]) -> None:
        ...
        if not channels or any(
            ...
            raise ValueError("query/gallery-key scorer requires evidence channel specifications")
        if len({item.name for item in channels}) != len(channels):
            raise ValueError("query/gallery-key scorer channel names must be unique")
        if sum(item.weight for item in channels) <= 0.0:
            raise ValueError("query/gallery-key scorer weights must have a positive sum")
        if sum(item.weight for item in channels if not item.optional) <= 0.0:
            raise ValueError("required query/gallery-key channels must retain positive weight")
    ...
    def score(self, query: RetrievalQuery, key: GalleryKey) -> QueryKeyScore:
        ...
        if set(query.availability) != expected_names:
            raise ValueError("query availability differs from query/gallery-key scorer channels")
        if set(key.availability) != expected_names:
            raise ValueError(
                "gallery-key availability differs from query/gallery-key scorer channels"
            )
        intersection = [
            channel
            for channel in self.channels
        ...
        if total_weight <= 0.0:
            raise RuntimeError(
                "no positively weighted query/gallery-key evidence intersection remains"
            )
        evidence = {
            channel.name: float(
